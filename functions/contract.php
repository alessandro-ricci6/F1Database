...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if ($_POST['action'] == 'updateStaffContract') {
        ...
        $eDate = date_create_from_format('Y-m-d', $_POST['eDate']);
        $sDate = date_create_from_format('Y-m-d', $_POST['sDate']);
        $db->addDriverContract($driverId, $teamId, $sDate->format('Y-m-d'), $eDate == NULL ? NULL : $eDate->format('Y-m-d'));
        ...
        $db->addStaffContract($staffId, $teamId, $role, $sDate->format('Y-m-d'), $eDate == NULL ? NULL : $eDate->format('Y-m-d'));
    }
}